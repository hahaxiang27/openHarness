...
- **👀 可观测过程**：运行状态写入 `.openharness/`，并可通过 `hc monitor` 查看过程。
...
3. **初始化并选择后端**：`hc init`（可用 `--backend opencode|claude|codex`）。
4. **启动主循环**：`hc start`。
5. **观察过程**：另开终端运行 `hc monitor`，打开只读监控页（默认 `127.0.0.1:8765`）。

若使用 **变更级 SDD 工作流**（需求写在 `input/changes/<change-id>/`），可先阅读 [docs/sdd-harness-generation-architecture.md](docs/sdd-harness-generation-architecture.md)，再用 `hc prd` / `hc spec` / `hc gen` 与 `hc change` 管理活动变更，最后对当前活动变更执行 `hc start`。
...
flowchart LR
  subgraph specLine [规格侧 SDD]
    A[自然语言需求] --> B[hc prd / spec / gen]
    B --> C["input/changes 与元数据"]
  end
  subgraph runLine [执行侧 Harness]
    D[hc start] --> E[Orchestrator 读状态]
...
人在回路：当出现需人工补充的信息时，会体现在 `missing_info.json` 等文件中，可配合 `hc start` 的暂停逻辑处理。
...
**安装 openHarness 包**（PyPI 包名 `openharness`，命令行入口仍为 `hc`）
...
<details>
<summary><strong>排错：找不到 hc、PEP 668 等</strong></summary>

- **找不到 `hc`**：把 Python 的 `Scripts`（Windows）或 `~/.local/bin`（Unix）加入 PATH；可用 `python -c "import sysconfig; print(sysconfig.get_path('scripts'))"` 查看脚本目录。
...
| 命令 | 说明 |
|------|------|
| `hc init` | 交互式初始化（分支、后端、`~/.openharness` 学习数据等） |
| `hc start` | 启动 Harness 主循环 |
| `hc status` | 查看项目指标与近期成功率 |
| `hc restore` | 从备份恢复配置（如 PR 前备份） |
| `hc uninstall` | 清理本机 Agent 安装与项目内 `.openharness` 等（按提示） |
| `hc prd` / `hc spec` / `hc gen` | 按自然语言生成或更新变更级 PRD、techspec、整套产物（见 SDD 文档） |
| `hc change` | 列出 / 切换当前活动变更 |
| `hc monitor` | 本地只读监控（`--host` / `--port` / `--open`） |
| `hc --version` | 版本信息 |
